...
(function(JE) {
    ...
    /**
     * Creates a Jellyseerr section similar to item-details recommendations
    ...
    function createDiscoverSection(results, title) {
        ...
        section.className = 'verticalSection emby-scroller-container jellyseerr-network-discovery-section';
        ...
        // Enable smooth native horizontal touch scrolling
    ...
    async function renderNetworkDiscovery() {
        ...
        const sectionTitle = JE.t
            ? (JE.t('jellyseerr_more_from_network', { network: studioInfo.name }) || `More from ${studioInfo.name}`)
            : `More from ${studioInfo.name}`;

        // Create and insert the section
        const section = createDiscoverSection(allResults.slice(0, 20), sectionTitle);
        if (section) {
            // Find the parent container to insert the section
            ...
            if (parentContainer && parentContainer.parentElement) {
                parentContainer.parentElement.insertBefore(section, parentContainer);
