...
moreInfoModal.open = async function(tmdbId, mediaType) {
    try {
        ...
        if (cached) {
            ...
            // Background: revalidate details and update status if changed.
            // The _renderActionsGen counter in renderActions prevents duplicate buttons
            // even if the TV request event handler also triggers renderActions concurrently.
        ...
        ratingsPromise.then((ratings) => {
            // Guard: only update if this modal is still the one we opened
            if (!currentModal || String(currentModal.dataset?.tmdbId) !== String(tmdbId)) return;
            if (ratings) {
...
moreInfoModal.prefetch = function(tmdbId, mediaType) {
    // Don't prefetch if already cached (cacheHas avoids unnecessary structuredClone)
    if (cacheHas(mediaType, tmdbId)) return;
...
async function fetchMediaDetails(tmdbId, mediaType, { skipCache = false } = {}) {
    ...
    // Skip dedup when skipCache is true (refresh button needs fresh data)
...
function buildRatingLogos(ratings, data, mediaType, tmdbId) {
    ...
    if (ratings?.rt?.criticsScore !== undefined) {
        ...
        const rtLink = ratings.rt.url || ratings.url || 'https://www.rottentomatoes.com';
        const rottenSvg = SVG_RT_ROTTEN;
        const freshSvg = SVG_RT_FRESH;
        ...
        if (ratings.rt.audienceScore !== undefined) {
            ...
            const popcornFull = SVG_POPCORN_FULL;
            const popcornSpilled = SVG_POPCORN_SPILLED;
            logos.push(`
...
function buildMediaFacts(data, mediaType, tmdbId) {
    ...
    const jellyfinSvg = SVG_JELLYFIN;
    const jellyfinSvg4k = SVG_JELLYFIN_4K;
                    ...
                    return `
                        <a is="emby-linkbutton" href="${escapeHtml(trailer.url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(trailer.name)}" class="trailer-item">
                            <div class="trailer-thumbnail">
                                ${thumbnailUrl ? `<img src="${thumbnailUrl}" alt="${escapeHtml(trailer.name)}" loading="lazy" />` : ''}
