...
(function (JE) {
    ...
    // Reactive lifecycle integration: register with moduleRegistry so
    // toggling ShowReviews / ShowUserReviews live runs teardown + re-init
    // without a page refresh. The ctx.dom selectors clean injected DOM on
    // disable; the onTeardown hook disconnects any reviews-owned observer.
    var ctx = JE.helpers ? JE.helpers.createModuleContext('reviews') : null;
    if (ctx) {
        ctx.dom('.tmdb-reviews-section');
        ctx.dom('.je-user-reviews-section');
        ctx.onTeardown(function() {
            if (JE.helpers) JE.helpers.disconnectObserver('reviews');
        });
    }

    if (JE.moduleRegistry && ctx) {
        JE.moduleRegistry.register('reviews', {
            configKeys: ['ShowReviews', 'ShowUserReviews', 'ReviewsExpandedByDefault'],
            enableKey: 'ShowReviews',
